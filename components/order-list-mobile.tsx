...
export default function OrderListMobile({
  ...
  return (
    <>
      ...
      {orders?.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((order, index) => (
        <div key={'order-' + order.id} className="grid grid-cols-2 gap-4">
          <div className="flex gap-2 col-span-2 items-center justify-between">
            <Label className="text-right text-muted-foreground">طلب رقم {order.id}</Label>
            <Button
              variant="outline"
              onClick={() => {
          ...
          <div className="flex flex-col gap-2">
            <Label className="text-right text-muted-foreground">التاريخ</Label>
          ...
          <div className="flex flex-col gap-2">
            <Label className="text-right text-muted-foreground">المبلغ الكلي</Label>
            <Label className="text-right">{order.total}DH</Label>
          </div>
          <div className="flex flex-col gap-2">
            <Label className="text-right text-muted-foreground">المبلغ المدفوع</Label>
            <Label className="text-right">{order.deposit}DH</Label>
            <Label className={`text-right ${order.total - order.deposit < 0 ? 'text-green-700' : 'text-red-700'}`}>
              ({order.total - order.deposit}DH)
            </Label>
          </div>

          <div className="flex flex-col gap-2 col-span-2">
            <Label className="text-right text-muted-foreground">التفاصيل</Label>
            <Label className="text-right">{order.notes}</Label>
          </div>

          {index !== orders.length - 1 && <Separator className="col-span-2 my-8 bg-zinc-500" />}
        </div>
      ))}
      {orders?.length && orders?.length > pageSize && (
        <PaginationComponent pageName="orderPage" pageCount={Math.ceil((orders?.length || 0) / pageSize)} />
      )}
    </>
  );
}
